...
export interface ChatTopBarParticipant {
  name: string;
  username: string;
  avatar: string;
  isVerified?: boolean;
...
interface ChatTopBarProps {
  participant: ChatTopBarParticipant;
  onBack: () => void;
...
export const ChatTopBar: React.FC<ChatTopBarProps> = ({
  ...
  return (
    <header className="sticky top-0 inset-x-0 z-40 bg-white/95 backdrop-blur-md border-b border-neutral-200/80 h-14 px-2 sm:px-4 flex items-center justify-between select-none">
      {/* 1. Left Section: ArrowLeft + Profile PFP + Name */}
      <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0 flex-1">
        ...
        {/* Profile PFP & Name Container (Clickable) */}
        <div
          onClick={handleProfileClick}
          className="flex items-center gap-2.5 min-w-0 flex-1 cursor-pointer py-1 group"
        >
          {/* Profile Picture (PFP) with Online Indicator */}
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11.5px] text-neutral-500 font-normal truncate leading-tight mt-0.5">
              {participant.isOnline
                ? 'Aktif sekarang'
                : participant.subtitle || `@${participant.username}`}
            </p>
          </div>
        </div>
      </div>

      {/* 2. Right Section: Option Icon (3-Dots) */}
