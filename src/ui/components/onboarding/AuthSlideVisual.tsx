        ...
        {regStep === 'otp' ? (
          <form onSubmit={handleVerifyOtp} className="space-y-6 pt-4 text-center animate-in fade-in slide-in-from-right-4 duration-300 ease-out">
            ...
            <div className="space-y-2">
              <div key={`otp-box-${shakeKey}`} className={`flex items-center justify-center gap-2 my-2 ${errors.otpCode ? 'do-shake' : ''}`}>
                {otpDigits.map((digit, idx) => (
                  <input
                    ...
                    className={`w-12 h-14 text-center text-xl font-bold rounded-2xl border transition-all shadow-xs focus:outline-none ${
                      errors.otpCode
                        ? 'border-rose-400 bg-rose-50/30 text-rose-600 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20'
                        : digit
                        ? 'border-[#1d64ec] bg-blue-50/20 text-slate-900 ring-2 ring-blue-500/20'
                        : 'border-neutral-200 bg-neutral-50/50 text-slate-900 focus:border-[#1d64ec] focus:bg-pure-white focus:ring-2 focus:ring-blue-500/20'
                    }`}
                  />
                ))}
              </div>
              ...
              {errors.otpCode && (
                <p className="text-[11px] text-rose-500 font-semibold flex items-center justify-center gap-1.5 leading-none animate-in fade-in duration-200">
